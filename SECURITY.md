...
- Only explicitly supplied task/log text, recent conversation and target count are sent with fixed safety/format instructions. Conversation history is bounded, treated as untrusted data and re-redacted in every preview. Passwords used for SSH are not model inputs. Redaction runs on the backend before a separate send confirmation. Heuristics and known-secret matching cannot guarantee removal of unknown, encoded, split or business-specific secrets. Human preview and the chosen provider's data retention policy remain important.
