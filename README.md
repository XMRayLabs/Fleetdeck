...
Agent 工作区把连续对话、命令审批、执行结果放在中间，服务器范围放在右侧，消息与日志附件放在底部。支持快捷问题、开始新对话及 Ctrl / ⌘ + Enter 预览。最多保留最近 6 轮对话，每轮各取最多 3,500 字参与下一次脱敏预览；离开页面即清空。历史建议不可直接执行，新操作仍需重新确认。改变 API 或模型后先保存，避免显示模型与实际调用模型不一致。
