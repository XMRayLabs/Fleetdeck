...
// 本地存储的 Key
const LAYOUT_STORAGE_KEY = 'fleetdeck_layout_config';
const SIDEBAR_STORAGE_KEY = 'fleetdeck_sidebar_config';
const LEGACY_LAYOUT_STORAGE_KEY = 'nexus_terminal_layout_config';
const LEGACY_SIDEBAR_STORAGE_KEY = 'nexus_terminal_sidebar_config';
...
function ensureNodeIds(node: LayoutNode | null): LayoutNode | null {
  ...
  async function initializeLayout() {
    ...
    if (!layoutLoadedFromBackend) {
      ...
      try {
        const savedLayout = localStorage.getItem(LAYOUT_STORAGE_KEY) ?? localStorage.getItem(LEGACY_LAYOUT_STORAGE_KEY);
        if (savedLayout) {
    ...
    if (!sidebarLoadedFromBackend) {
        ...
        try {
            const savedSidebars = localStorage.getItem(SIDEBAR_STORAGE_KEY) ?? localStorage.getItem(LEGACY_SIDEBAR_STORAGE_KEY);
            if (savedSidebars) {
