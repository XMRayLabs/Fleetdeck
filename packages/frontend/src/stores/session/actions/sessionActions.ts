...
export const openNewSession = (
    ...
    ephemeralCredentials?: { password?: string; passphrase?: string }
) => {
...
export const handleConnectRequest = (
  ...
  if (connection.type === 'RDP') {
    ...
    let ephemeralCredentials: { password?: string; passphrase?: string } | undefined;
    if (connection.credential_mode === 'prompt') {
      const label = connection.auth_method === 'key'
        ? `请输入 ${connection.name || connection.host} 的密钥口令（无口令可留空）`
        : `请输入 ${connection.name || connection.host} 的 SSH 密码`;
      const secret = window.prompt(label);
      if (secret === null || (connection.auth_method === 'password' && secret.length === 0)) return;
      ephemeralCredentials = connection.auth_method === 'key' ? { passphrase: secret } : { password: secret };
    }
    ...
    if (!activeAndDisconnected) {
      ...
      openNewSession(connection, { connectionsStore, t }, undefined, ephemeralCredentials);
      router.push({ name: 'Workspace' });
    }
  }
};
