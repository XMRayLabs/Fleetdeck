...
export const useSettingsStore = defineStore('settings', () => {
  ...
  async function loadInitialSettings() {
    ...
    try {
      ...
      const localQcRowSizeMultiplier = localStorage.getItem('fleetdeck_quickCommandRowSizeMultiplier')
        ?? localStorage.getItem('nexus_quickCommandRowSizeMultiplier');
      ...
      const localQcCompactMode = localStorage.getItem('fleetdeck_quickCommandsCompactMode')
        ?? localStorage.getItem('nexus_quickCommandsCompactMode');
  ...
  async function updateSetting(key: keyof SettingsState, value: string | boolean) {
    ...
    try {
        ...
        if (key === 'quickCommandsCompactMode' && (String(value) === 'true' || String(value) === 'false')) {
          try {
            localStorage.setItem('fleetdeck_quickCommandsCompactMode', String(value));
  ...
  async function updateQuickCommandRowSizeMultiplier(multiplier: number) {
    ...
    try {
      ...
      // --- 保存到 localStorage ---
      try {
        localStorage.setItem('fleetdeck_quickCommandRowSizeMultiplier', multiplierString);
