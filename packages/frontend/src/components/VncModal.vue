...
const handleConnection = async () => {
  ...
  try {
    ...
    if (props.connection.credential_mode === 'prompt') {
      const entered = window.prompt(`请输入 ${props.connection.name || props.connection.host} 的 VNC 密码`);
      if (entered === null || entered.length === 0) {
        connectionStatus.value = 'disconnected';
        statusMessage.value = '已取消连接';
        return;
      }
      temporaryPassword = entered;
    }
    // Pass width and height to the token generation, backend will forward to gateway
    const token = await connectionsStore.getVncSessionToken(props.connection.id, desiredModalWidth.value, desiredModalHeight.value, temporaryPassword);
    if (!token) {
