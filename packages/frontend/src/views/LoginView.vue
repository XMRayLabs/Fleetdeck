...
<template>
  ...
  <div class="flex items-center justify-center min-h-screen bg-background p-4 bg-[radial-gradient(circle_at_top_left,color-mix(in_srgb,var(--color-primary)_15%,transparent),transparent_35%)]">
    ...
    <div class="flex w-full max-w-5xl rounded-3xl shadow-2xl overflow-hidden bg-background/95 border border-border backdrop-blur-xl">
      ...
      <div class="hidden md:flex w-2/5 bg-gradient-to-br from-primary via-primary to-primary-dark flex-col items-center justify-center p-10 text-white relative">
         <!-- Subtle pattern or overlay could go here -->
         <div class="z-10 text-center">
           <div class="mx-auto mb-6 grid h-20 w-20 place-items-center rounded-3xl border border-white/30 bg-white/15 text-2xl font-black shadow-2xl backdrop-blur">FD</div>
      ...
      <div class="w-full md:w-3/5 flex flex-col justify-center p-8 sm:p-12">
        <!-- Mobile Logo (optional) -->
         <div class="flex justify-center mb-6 md:hidden">
           <div class="grid h-16 w-16 place-items-center rounded-2xl bg-primary text-xl font-black text-white shadow-lg shadow-primary/20">FD</div>
         </div>
        ...
        <form @submit.prevent="handleSubmit" class="space-y-5"> <!-- Reduced space slightly -->
          ...
          <div v-if="!loginRequires2FA" class="space-y-6">
            <div>
              ...
              <input type="text" id="username" v-model="credentials.username" required maxlength="128" autocomplete="username webauthn" :disabled="isLoading"
            ...
            <div>
              ...
              <input type="password" id="password" v-model="credentials.password" required maxlength="256" autocomplete="current-password" :disabled="isLoading"
                     class="w-full px-4 py-3 border border-border/50 rounded-lg bg-input text-foreground text-base shadow-sm focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary transition duration-150 ease-in-out disabled:bg-gray-100 disabled:cursor-not-allowed" />
            </div>
