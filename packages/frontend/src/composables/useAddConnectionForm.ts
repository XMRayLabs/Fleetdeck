...
export function useAddConnectionForm(props: AddConnectionFormProps, emit: AddConnectionFormEmits) {
  ...
  const initialFormData = {
    ...
    credential_mode: 'saved' as 'saved' | 'prompt',
    password: '',
    private_key: '',
    passphrase: '',
  ...
  watch(connectionToEdit, (newVal) => {
      ...
      if (newVal) {
          ...
          formData.credential_mode = newVal.credential_mode || 'saved';
  ...
  const handleSubmit = async () => {
    ...
    if (formData.type === 'SSH') {
        if (!isEditMode.value) {
            if (formData.auth_method === 'password' && formData.credential_mode === 'saved' && !formData.password && !formData.host.includes('~')) {
            ...
            if (formData.auth_method === 'password' && formData.credential_mode === 'saved' && !formData.password && connectionToEdit.value?.auth_method !== 'password') {
        ...
        if (formData.credential_mode === 'saved' && !formData.password && !formData.host.includes('~') && (!isEditMode.value || connectionToEdit.value?.credential_mode === 'prompt')) {
            uiNotificationsStore.showError(t('connections.form.errorPasswordRequired'));
            return;
        }
    } else if (formData.type === 'VNC') {
        if (formData.credential_mode === 'saved' && !formData.vncPassword && !formData.host.includes('~') && (!isEditMode.value || connectionToEdit.value?.credential_mode === 'prompt')) {
    ...
    if (!isEditMode.value && formData.host.includes('~')) {
        ...
        if (Array.isArray(parsedIpsResult)) {
            ...
            if (formData.type === 'SSH' && formData.auth_method === 'password' && formData.credential_mode === 'saved' && !formData.password) {
                uiNotificationsStore.showError(t('connections.form.errorPasswordRequiredForBatchSSH', '批量添加 SSH (密码认证) 连接时，必须提供密码。'));
                return;
            }
            if (formData.type === 'RDP' && formData.credential_mode === 'saved' && !formData.password) {
                uiNotificationsStore.showError(t('connections.form.errorPasswordRequiredForBatchRDP', '批量添加 RDP 连接时，必须提供密码。'));
                return;
            }
            if (formData.type === 'VNC' && formData.credential_mode === 'saved' && !formData.vncPassword) {
            ...
            for (let i = 0; i < ips.length; i++) {
                ...
                if (formData.type === 'SSH') {
                    ...
                    dataForThisIp.credential_mode = formData.credential_mode;
    ...
    if (formData.type === 'SSH') {
        ...
        dataToSend.credential_mode = formData.credential_mode;
        if (formData.auth_method === 'password') {
