    ...
    if (url === null || url === undefined) {
      ...
      if (url.trim() === '') {
        ...
        if (!/^https:\/\/github\.com\/[^/]+\/[^/]+(?:\/tree\/[^/]+(?:\/.*)?)?\/?$/.test(url)) {
          throw new Error('远程 HTML 主题仅支持 GitHub 官方 HTTPS 仓库地址。');
        }
...
export const updateRemoteHtmlPresetsRepositoryUrl = async (url: string | null): Promise<void> => {
    try {
        ...
        if (url && typeof url === 'string' && url.trim() !== '') {
            // 简单的 URL 验证，可以根据需要增强
            if (!/^https:\/\/github\.com\/[^/]+\/[^/]+(?:\/tree\/[^/]+(?:\/.*)?)?\/?$/.test(url)) {
                throw new Error('无效的 GitHub HTTPS 仓库链接。');
            }
        } else if (url === '') {
...
const parseGitHubRepoUrl = (repoUrl: string): { user: string; repo: string; repoPath: string; ref: string } | null => {
    ...
    const githubUrlRegex = /^https:\/\/github\.com\/([A-Za-z0-9_.-]+)\/([A-Za-z0-9_.-]+)(?:\/tree\/([A-Za-z0-9_.-]+)\/?([A-Za-z0-9_./-]*))?\/?$/;
...
export const getRemoteHtmlPresetContent = async (fileUrl: string): Promise<string> => {
    ...
    try {
        ...
        throw new Error('远程主题文件 URL 无效。');
    }
    if (parsedUrl.protocol !== 'https:' || parsedUrl.hostname !== 'raw.githubusercontent.com') {
        throw new Error('远程主题文件只允许从 GitHub 官方 raw 域名下载。');
    }
