...
export class NotificationService {
  ...
  private async _testEmailSetting(
    ...
    const mailOptions: Mail.Options = {
      ...
      subject: i18next.t(testSubjectKey, {
        lng: userLang,
        defaultValue: "FleetDeck Test Notification ({event})",
        eventDisplay: eventDisplayName,
      }),
      text: i18next.t(testEmailBodyKey, {
        lng: userLang,
        timestamp: new Date().toISOString(),
        defaultValue: `This is a test email from FleetDeck for event '{{event}}'.\n\nIf you received this, your SMTP configuration is working.\n\nTimestamp: {{timestamp}}`,
        eventDisplay: eventDisplayName,
      }),
      html: i18next.t(testEmailBodyHtmlKey, {
        lng: userLang,
        timestamp: new Date().toISOString(),
        defaultValue: `<p>This is a test email from <b>FleetDeck</b> for event '{{event}}'.</p><p>If you received this, your SMTP configuration is working.</p><p>Timestamp: {{timestamp}}</p>`,
        eventDisplay: eventDisplayName,
      }),
    };
    ...
    const testPayload: NotificationPayload = {
      ...
      details: {
        message: i18next.t(testWebhookDetailsKey, {
          lng: userLang,
          defaultValue:
            "This is a test notification from FleetDeck (Webhook).",
        }),
      },
    };
  ...
  private async _testTelegramSetting(
    ...
    const detailsOptions = {
      lng: userLang,
      defaultValue:
        "Fallback: This is a test notification from FleetDeck (Telegram).",
    };
    ...
    const defaultMessageTemplateFromI18n = i18next.t(templateKeyWithNamespace, {
      lng: userLang,
      defaultValue: `Fallback Template: *FleetDeck Test Notification*\nEvent: \`{event}\`\nTimestamp: {timestamp}\nDetails:\n\`\`\`\n{details}\n\`\`\``,
    });
    console.log(
