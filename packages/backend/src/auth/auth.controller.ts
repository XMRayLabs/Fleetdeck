...
const isAcceptableNewPassword = (password: unknown): password is string => typeof password === 'string'
    && password.length >= 12
    && password.length <= 128
...
const encryptTotpSecret = (secret: string): string => `enc:v1:${encrypt(secret)}`;
const decryptTotpSecret = (stored: string): string => stored.startsWith('enc:v1:')
    ? decrypt(stored.slice('enc:v1:'.length))
    : stored;
...
export const login = async (req: Request, res: Response): Promise<void> => {
    ...
    if (typeof username !== 'string' || username.length < 1 || username.length > 128 || typeof password !== 'string' || password.length < 1 || password.length > 256) {
...
export const changePassword = async (req: Request, res: Response): Promise<void> => {
    ...
    if (!isAcceptableNewPassword(newPassword)) {
        res.status(400).json({ message: '新密码须为 12 至 128 个字符，且 UTF-8 编码不超过 72 字节。' });
        return;
    }
    if (currentPassword === newPassword) {
...
export const setup2FA = async (req: Request, res: Response): Promise<void> => {
    ...
    try {
        ...
        const secret = speakeasy.generateSecret({
            length: 20,
            name: `FleetDeck (${username})`
        });

        req.session.tempTwoFactorSecret = encryptTotpSecret(secret.base32);
...
export const verifyAndActivate2FA = async (req: Request, res: Response): Promise<void> => {
    const { token } = req.body;
    const userId = req.session.userId;
    const encryptedTempSecret = req.session.tempTwoFactorSecret;
    ...
    try {
        const db = await getDbInstance();
        const tempSecret = decryptTotpSecret(encryptedTempSecret);
        const verified = speakeasy.totp.verify({
            secret: tempSecret,
            encoding: 'base32',
            token: token,
        ...
        if (verified) {
            ...
            const result = await runDb(db,
                ...
                [encryptTotpSecret(tempSecret), now, userId]
            );
...
export const setupAdmin = async (req: Request, res: Response): Promise<void> => {
    ...
    if (!/^[A-Za-z0-9_.-]{3,64}$/.test(username)) {
        res.status(400).json({ message: '用户名须为 3 至 64 位字母、数字、点、下划线或连字符。' });
        return;
    }
    if (password !== confirmPassword) {
     ...
     if (!isAcceptableNewPassword(password)) {
        res.status(400).json({ message: '密码须为 12 至 128 个字符，且 UTF-8 编码不超过 72 字节。' });
        return;
    }
