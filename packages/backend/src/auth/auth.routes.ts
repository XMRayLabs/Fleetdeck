...
const authenticationLimiter = rateLimit({
  ...
  legacyHeaders: false,
  message: { message: '登录尝试过于频繁，请稍后重试。' },
});
...
router.post('/setup', authenticationLimiter, setupAdmin);

// POST /api/v1/auth/login - 用户登录接口 (添加黑名单检查)
router.post('/login', authenticationLimiter, ipBlacklistCheckMiddleware, login);
...
router.post('/login/2fa', authenticationLimiter, ipBlacklistCheckMiddleware, verifyLogin2FA);
...
router.post('/passkey/authentication-options', authenticationLimiter, generatePasskeyAuthenticationOptionsHandler);


// POST /api/v1/auth/passkey/authenticate - 验证 Passkey 并登录用户 (公开)
router.post('/passkey/authenticate', authenticationLimiter, ipBlacklistCheckMiddleware, verifyPasskeyAuthenticationHandler);
