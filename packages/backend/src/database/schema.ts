...
CREATE TABLE IF NOT EXISTS connections (
    ...
    credential_mode TEXT NOT NULL CHECK(credential_mode IN ('saved', 'prompt')) DEFAULT 'saved',
    encrypted_password TEXT NULL,
    encrypted_private_key TEXT NULL,
    encrypted_passphrase TEXT NULL,
...
// Batch orchestration jobs. Command payloads and command output are encrypted
// before they reach SQLite; only non-sensitive scheduling metadata is stored in
// plaintext so that the queue can be inspected safely.
export const createOrchestrationJobsTableSQL = `
CREATE TABLE IF NOT EXISTS orchestration_jobs (
    id TEXT PRIMARY KEY NOT NULL,
    name TEXT NOT NULL,
    type TEXT NOT NULL CHECK(type IN ('command', 'upload')),
    status TEXT NOT NULL CHECK(status IN ('queued', 'running', 'success', 'partial', 'failed', 'cancelled')) DEFAULT 'queued',
...
export const createOrchestrationJobTargetsTableSQL = `
CREATE TABLE IF NOT EXISTS orchestration_job_targets (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    ...
    encrypted_stdout TEXT NULL,
    encrypted_stderr TEXT NULL,
    error TEXT NULL,
    started_at INTEGER NULL,
    finished_at INTEGER NULL,
...
// Reusable orchestration playbooks. The definition contains commands,
// variable defaults, and remote paths, so it is encrypted as a single value.
export const createOrchestrationPlaybooksTableSQL = `
...
export const createOrchestrationPlaybookFilesTableSQL = `
CREATE TABLE IF NOT EXISTS orchestration_playbook_files (
    id TEXT PRIMARY KEY NOT NULL,
    ...
    created_at INTEGER NOT NULL DEFAULT (strftime('%s', 'now')),
    FOREIGN KEY (playbook_id) REFERENCES orchestration_playbooks(id) ON DELETE CASCADE
);
`;
