...
,
{
    ...
    {
        id: 11,
        name: 'Add prompt-on-connect credential mode',
        check: async (db: Database): Promise<boolean> => {
            return !(await columnExists(db, 'connections', 'credential_mode'));
        },
        sql: `
            ALTER TABLE connections ADD COLUMN credential_mode TEXT NOT NULL DEFAULT 'saved'
                CHECK(credential_mode IN ('saved', 'prompt'));
        `
    }
];
