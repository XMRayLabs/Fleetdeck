...
// Preserve existing installations during the product rename. This is a one-way
// filename migration only; the database schema and encrypted values are unchanged.
if (!fs.existsSync(dbPath) && fs.existsSync(legacyDbPath)) {
    fs.renameSync(legacyDbPath, dbPath);
    console.log(`[Database] Migrated legacy database filename to ${dbFilename}.`);
}
