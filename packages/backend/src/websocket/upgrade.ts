...
export function initializeUpgradeHandler(
    ...
    server.on('upgrade', (request: Request, socket, head) => {
        ...
        const expectedOrigin = process.env.APP_ORIGIN || process.env.RP_ORIGIN || `${forwardedProto}://${request.headers.host}`;
        try {
        ...
        sessionParser(request, {} as any, () => {
            // --- 认证检查 ---
            if (!request.session || !request.session.userId || !request.session.username || request.session.requiresTwoFactor) {
