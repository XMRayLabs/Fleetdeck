...
export function handleRdpProxyConnection(
    ...
    // The URL contains the encrypted connection token. Do not copy it into logs.
    console.log(`WebSocket: Remote Desktop Proxy for ${ws.username} connecting to ${cleanRemoteGatewayWsBaseUrl}.`);
    ...
    rdpWs.on('error', (error) => {
         console.error(`[RDP 代理 RDP WS 错误] 用户: ${ws.username}, 会话: ${ws.sessionId}, 网关连接失败:`, error);
