...
export async function handleSshConnect(
    ...
    try {
        ...
        const credentials = payload?.credentials && typeof payload.credentials === 'object'
            ? {
                password: typeof payload.credentials.password === 'string' ? payload.credentials.password.slice(0, 4096) : undefined,
                passphrase: typeof payload.credentials.passphrase === 'string' ? payload.credentials.passphrase.slice(0, 4096) : undefined,
            }
            : undefined;
        connInfo = await SshService.getConnectionDetails(dbConnectionId, credentials);
