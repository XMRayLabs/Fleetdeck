...
export const findAllConnectionsWithTags = async (): Promise<ConnectionWithTags[]> => {
    const sql = `
        SELECT
            c.id, c.name, c.type, c.host, c.port, c.username, c.auth_method, c.credential_mode, c.proxy_id, c.proxy_type, c.ssh_key_id, c.notes, c.jump_chain,
...
export const findFullConnectionById = async (id: number): Promise<FullConnectionDbRow | null> => {
 ...
 export const findConnectionByName = async (name: string): Promise<ConnectionBase | null> => {
     const sql = `SELECT id, name, type, host, port, username, auth_method, credential_mode, proxy_id, proxy_type, ssh_key_id, notes, jump_chain, created_at, updated_at, last_connected_at FROM connections WHERE name = ?`;
     try {
         const db = await getDbInstance();
...
export const createConnection = async (data: Omit<FullConnectionData, 'id' | 'created_at' | 'updated_at' | 'last_connected_at' | 'tag_ids'>): Promise<number> => {
    ...
    const sql = `
        INSERT INTO connections (name, type, host, port, username, auth_method, credential_mode, encrypted_password, encrypted_private_key, encrypted_passphrase, proxy_id, proxy_type, ssh_key_id, notes, jump_chain, created_at, updated_at)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;
