...
export const exportConnections = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const filename = `fleetdeck-connections-${timestamp}.zip`;
...
export const getRdpSessionToken = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const temporaryPassword = typeof req.body?.password === 'string' ? req.body.password : undefined;
        if ((temporaryPassword?.length || 0) > 4096) {
            res.status(400).json({ message: '临时密码不能超过 4,096 个字符。' });
            return;
        }
        const sessionPassword = connection.credential_mode === 'prompt' ? temporaryPassword : decryptedPassword;

        // 3. 验证 RDP 连接是否使用密码认证
        if (connection.auth_method !== 'password' || !sessionPassword) {
        ...
        const guacamoleToken = await GuacamoleService.getRemoteDesktopToken('rdp', connection, sessionPassword, rdpWidth, rdpHeight, rdpDpi);
...
export const getVncSessionToken = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const guacamoleToken = await GuacamoleService.getRemoteDesktopToken('vnc', connection, sessionPassword, initialWidth, initialHeight);
