...
export const createConnection = async (input: CreateConnectionInput): Promise<ConnectionWithTags> => {
    ...
    // Never log the raw input: it may contain passwords, private keys, or
    // passphrases. Keep only non-sensitive routing metadata for diagnostics.
    console.log('[Service:createConnection] Creating connection:', {
        type: input.type,
        host: input.host,
        port: input.port,
    ...
    const credentialMode: 'saved' | 'prompt' = input.credential_mode === 'prompt' ? 'prompt' : 'saved';
    ...
    if (connectionType === 'SSH') {
        ...
        if (input.auth_method === 'password' && credentialMode === 'saved' && !input.password) {
        ...
        if (credentialMode === 'saved' && !input.password) {
    ...
    if (connectionType === 'SSH') {
        ...
        if (input.auth_method === 'password') {
            encryptedPassword = credentialMode === 'saved' && input.password ? encrypt(input.password) : null;
            ...
            if (input.ssh_key_id) {
                ...
                if (input.passphrase && credentialMode === 'saved') {
...
export const updateConnection = async (id: number, input: UpdateConnectionInput): Promise<ConnectionWithTags | null> => {
    ...
    const finalCredentialMode: 'saved' | 'prompt' = input.credential_mode || currentFullConnection.credential_mode || 'saved';
    ...
    if (input.credential_mode !== undefined) {
        dataToUpdate.credential_mode = input.credential_mode;
        if (input.credential_mode === 'prompt') {
    ...
    if (targetType === 'SSH') {
        ...
        if (finalAuthMethod === 'password') {
            if (finalCredentialMode === 'saved' && !input.password && !currentFullConnection.encrypted_password) {
                throw new Error('切换到安全保存模式时需要提供 password。');
            }
            ...
            if (input.password !== undefined) { // Check if password was provided in input
                 ...
                 // Encrypt if password is not empty, otherwise set to null (to clear)
                  dataToUpdate.encrypted_password = finalCredentialMode === 'saved' && input.password ? encrypt(input.password) : null;
            ...
            if (input.ssh_key_id !== undefined) {
                ...
                if (input.ssh_key_id === null) {
                    ...
                    if (input.private_key === undefined) {
                        ...
                        dataToUpdate.encrypted_passphrase = finalCredentialMode === 'saved' && input.passphrase ? encrypt(input.passphrase) : null;
                    }
                } else {
                    // Validate the provided ssh_key_id
                    const keyExists = await SshKeyService.getSshKeyDbRowById(input.ssh_key_id);
        ...
        if (finalCredentialMode === 'saved' && !input.password && !currentFullConnection.encrypted_password) {
            throw new Error('切换到安全保存模式时需要提供 RDP password。');
        }
        ...
        if (finalCredentialMode === 'saved' && !input.password && !currentFullConnection.encrypted_password) {
            throw new Error('切换到安全保存模式时需要提供 VNC password。');
        }
        if (input.password !== undefined) { // Check if password was provided
            dataToUpdate.encrypted_password = finalCredentialMode === 'saved' && input.password ? encrypt(input.password) : null;
            needsCredentialUpdate = true;
...
export const cloneConnection = async (originalId: number, newName: string): Promise<ConnectionWithTags> => {
    ...
    const dataForNewConnection: Omit<ConnectionRepository.FullConnectionData, 'id' | 'created_at' | 'updated_at' | 'last_connected_at' | 'tag_ids'> = {
        ...
        credential_mode: originalFullConnection.credential_mode || 'saved',
