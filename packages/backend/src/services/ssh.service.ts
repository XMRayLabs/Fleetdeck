 ...
 */
export const getConnectionDetails = async (connectionId: number, ephemeral?: EphemeralCredentials): Promise<DecryptedConnectionDetails> => {
    ...
    try {
        const fullConnInfo: DecryptedConnectionDetails = {
            ...
            credential_mode: typedRawConnInfo.credential_mode || 'saved',
            password: undefined,
        ...
        if (fullConnInfo.credential_mode === 'prompt') {
            if (fullConnInfo.auth_method === 'password') {
                if (!ephemeral?.password) {
                    throw new Error('This server requires a password for each connection.');
                }
                fullConnInfo.password = ephemeral.password;
            } else {
                // An empty passphrase is valid for an unencrypted private key.
                fullConnInfo.passphrase = ephemeral?.passphrase || undefined;
            }
        }
